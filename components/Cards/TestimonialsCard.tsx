...
const TestimonialsCard: FC<ITestimonialsCard> = ({
	...
	return (
		<>
			<div className="flex flex-col items-center justify-center p-6 h-full min-h-[300px] md:min-h-[225px] lg:min-h-[300px] bg-white border-2 border-primary-default border-solid">
				<motion.div
					initial={initial}
					variants={stagger}
					whileInView="animate"
				...
				>
					<Paragraph
						...
						tailwindStyling="max-w-full max-w-sm mx-auto lg:mx-0 text-tiny lg:text-tiny text-primary-default text-center lg:text-left"
					/>
					<div className="flex items-center justify-center my-2 gap-4">
						<div className="w-full mb-1 h-[2px] bg-grey hidden sm:block" />
						...
						<div className="flex flex-row md:flex-wrap items-center justify-between px-0 md:px-5 -m-2 gap-0 md:gap-4">
							<div className="flex flex-row md:flex-col lg:flex-row items-center justify-between">
								<div className="w-auto p-2">
									<Image
										className={`${
											image?.sourceUrl ? "bg-transparent" : "bg-lightGreyTwo"
										} rounded-full m-auto lg:mx-0 w-16 h-16 object-cover object-center`}
										alt={`${image?.altText}`}
										src={
											image?.sourceUrl
												? image?.sourceUrl
